...
if __name__ == '__main__':
    ...
    # 建立 各规模疫情前_后每日用电能力指数表
    scale_rq_ydnlzs1 = pd.DataFrame(columns=('规模', '日期', '用电能力指数'))

    # 建立 各规模疫情前_后整体用电能力变化表
    scale_jd_ydnlzs_ztbhl = pd.DataFrame(columns=('规模', '疫情前平均用电能力指数', '疫情后平均用电能力指数', '整体变化率'))
        ...
        for j in range(yyq_data.shape[0]):
            ...
            if rq != datetime.date(2020, 2, 29):
                ...
                if zcyly_data.empty == False:
                    ...
                    # 获取 此日总用电量1 (2020)
                    zydl_data = (data[(data['data_date'] == rq) & (data['scale'] == scale)])

                    # 获取 此日总用电量2 (2019)
                    zydl_data2 = (data[(data['data_date'] == rq2) & (data['scale'] == scale)])
                    ...
                    if ((zydl_data.empty == False) & (zydl_data2.empty == False)):
                        ...
                        if zydl2 != 0:
                            ...
                            # 数据入表
                            scale_rq_ydnlzs1 = scale_rq_ydnlzs1.append(pd.DataFrame({'规模': [scale], '日期': [rq3], '用电能力指数': [ydnlzs]}),
                                                         ignore_index=True)
                ...
                if zcyly_data.empty == False:
                    ...
                    if ((zydl_data.empty == False) & (zydl_data2.empty == False)):
                        ...
                        if zydl2 != 0:
                            ...
                            # 数据入表
                            scale_rq_ydnlzs1 = scale_rq_ydnlzs1.append(
                                pd.DataFrame({'规模': [scale], '日期': [rq3], '用电能力指数': [ydnlzs]}),
                                ignore_index=True)
    scale_rq_ydnlzs1.to_excel(f'{listdata_addr}各规模疫情前_后每日用电能力指数表.xlsx')
    ...
    for i in range(scale_data.shape[0]):
        ...
        # 获取该养老院 疫情前的用电能力指数
        ydnlzs1_data = scale_rq_ydnlzs1[
            (scale_rq_ydnlzs1['规模'] == scale) & (scale_rq_ydnlzs1['日期'] >= '01月01日') & (scale_rq_ydnlzs1['日期'] <= '01月22日')]

        # 获取该地市 疫情后的用电能力指数
        ydnlzs2_data = scale_rq_ydnlzs1[
            (scale_rq_ydnlzs1['规模'] == scale) & (scale_rq_ydnlzs1['日期'] >= '04月08日') & (scale_rq_ydnlzs1['日期'] <= '04月29日')]
        ...
        if (ydnlzs1_data.empty == False) & (ydnlzs2_data.empty == False):
            ...
            # 数据入表
            scale_jd_ydnlzs_ztbhl = scale_jd_ydnlzs_ztbhl.append(
                pd.DataFrame({'规模': [scale], '疫情前平均用电能力指数': [ydnlzs1], '疫情后平均用电能力指数': [ydnlzs2]}),
                ignore_index=True)

            scale_jd_ydnlzs_ztbhl.eval('''
                        整体变化率=100*(疫情后平均用电能力指数-疫情前平均用电能力指数)/疫情前平均用电能力指数
                        ''', inplace=True)
    scale_jd_ydnlzs_ztbhl.to_excel(f'{listdata_addr}各规模整体用电能力指数变化表.xlsx')